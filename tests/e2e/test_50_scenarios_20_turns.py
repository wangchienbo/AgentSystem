"""50 User Scenarios × 20 Turns — Architecture-Level E2E Test.

Tests the Phase P invocation architecture end-to-end WITHOUT relying on
external LLM APIs (which have rate limits). Uses direct dispatcher + runtime
layer + audit + model store to verify all Phase P pipeline components across
50 scenarios with 20 turns each.
"""
...
SCENARIOS = [
    ...
    {"id": "S12", "name": "App批量操作", "user_id": "user_lifecycle_07", "turns": [
        "帮我创建三个App：博客、论坛、Wiki", "三个都创建好了吗？", "列出我所有的App",
        "全部启动", "都启动成功了吗？", "查看每个App的状态", "博客App能发帖吗？",
        "帮我发一篇测试博文", "论坛App能创建板块吗？", "创建一个技术讨论板块",
        "WikiApp能创建页面吗？", "创建一个关于AgentSystem的页面", "三个App的存储用量各是多少？",
        "哪个App占用最多？", "停止博客App", "停止论坛App", "停止WikiApp",
        "确认都停了", "只重新启动WikiApp", "WikiApp单独运行正常吗？",
    ]},
    ...
    {"id": "S25", "name": "多轮-异常恢复", "user_id": "user_context_10", "turns": [
        "创建一个文档编辑器App", "打开一个新文档", "写一段文字：今天天气真好", "保存文档",
        "关闭文档", "重新打开", "内容还在吗？", "继续编辑：适合出去玩", "保存修改",
        "哎呀，误删了前面的内容", "能撤销吗？", "撤销后内容恢复了？", "查看版本历史",
        "恢复到第一个版本", "恢复成功了吗？", "添加标题：春日随笔", "设置字体大小",
        "导出为Markdown", "导出为PDF", "两个版本都保存好了吗？",
    ]},
    ...
    {"id": "S36", "name": "Skill-安装与使用", "user_id": "user_skill_01", "turns": [
        "查看可用的Skill列表", "安装一个代码审查Skill", "安装成功了吗？", "查看已安装的Skill",
        "用代码审查Skill检查一段代码", "代码是 def hello(): print('hello')", "审查结果如何？",
        "有改进建议吗？", "按照建议修改代码", "再次审查", "这次通过了吗？",
        "安装一个翻译Skill", "用翻译Skill翻译一段话", "翻译质量如何？",
        "卸载代码审查Skill", "确认卸载", "查看剩余Skill", "安装一个数据分析Skill",
        "用数据分析Skill分析一组数据", "分析结果合理吗？",
    ]},
    ...
    {"id": "S41", "name": "系统-状态监控", "user_id": "user_system_01", "turns": [
        "系统状态", "有多少个App在运行？", "列出所有App", "每个App的状态",
        "系统总内存使用", "CPU使用率", "磁盘空间", "网络连接数", "有哪些用户在线？",
        "系统运行了多久？", "最近有错误吗？", "查看系统日志", "日志级别是什么？",
        "修改日志级别为DEBUG", "触发一个操作", "查看DEBUG日志", "能看到详细信息吗？",
        "恢复日志级别为INFO", "生成系统健康报告", "系统健康吗？",
    ]},
...
# ---------------------------------------------------------------------------
# Architecture-level runner (no LLM dependency)
# ---------------------------------------------------------------------------
...
def _ensure_asset(asset_center: AssetCenterService, runtime_center: RuntimeCenter, user_id: str) -> str:
    """Ensure a test asset exists for the user in both asset_center and runtime_center."""
    asset_id = f"asset:{user_id}:v1"
    try:
        asset_center.register_asset(
            AssetDescriptorRecord(
                ...
                asset_id=asset_id,
                kind="system_asset",
                summary=f"Test asset for {user_id}",
                detail="E2E test",
    ...
    try:
        runtime_center.register_asset(
            AssetDescriptor(
                ...
                name=user_id,
                description=f"Test asset for {user_id}",
            ),
            method_mappings={
                "run": lambda **kw: {
                    "ok": True,
                    "result": f"executed for {user_id}",
                    "user_id": user_id,
                    "turn": kw.get("__invocation_envelope__").request_id if "__invocation_envelope__" in kw else "",
                }
            },
        )
    except Exception:
...
def run_scenario(
    ...
    runtime_layer: AssetInvocationRuntimeLayer,
    audit_store: InvocationAuditStore,
    model_store: ModelInvocationStore,
) -> ScenarioResult:
    ...
    for idx, message in enumerate(scenario["turns"]):
        ...
        try:
            envelope = InvocationRequestEnvelope(
                request_id=f"{session_id}_t{idx+1}",
                target_id=asset_id,
                target_type="system_asset",
                method="run",
                session=InvocationSessionRef(
                    upstream_session_id=session_id,
                    root_session_id=f"root_{user_id}",
                    parent_session_id=session_id if idx > 0 else None,
                ),
            )

            # Phase P pipeline: runtime layer → dispatcher → audit
            binding = runtime_layer.before_invoke(envelope)
            dispatch_result = dispatcher.dispatch_from_envelope(envelope)

            # Record audit trail
            audit_store.record(
                envelope=envelope,
                response=None,
                binding_resolution_mode=binding.mode,
                resolved_local_session_id=binding.local_session_id,
                downstream_call_links=[{"target": asset_id, "method": "run"}],
            )

            elapsed = (time.monotonic() - t0) * 1000
            ok = dispatch_result.get("ok", False)
            content = str(dispatch_result.get("execution", {}).get("result", ""))[:200] if ok else ""

            tr = TurnResult(turn_index=idx + 1, message=message, ok=ok, content_preview=content, elapsed_ms=elapsed)
        except Exception as exc:
            elapsed = (time.monotonic() - t0) * 1000
            tr = TurnResult(turn_index=idx + 1, message=message, ok=False, error=f"{type(exc).__name__}: {exc}", elapsed_ms=elapsed)
...
def main():
    print("[E2E-ARCH] Initializing Phase P architecture test ...")

    # Phase P services (isolated, no build_runtime needed)
    ...
    print(f"[E2E-ARCH] Running {len(SCENARIOS)} scenarios × 20 turns = {len(SCENARIOS)*20} total ...")
    all_results: list[ScenarioResult] = []
    grand_start = time.monotonic()

    for idx, scenario in enumerate(SCENARIOS):
        _ensure_asset(asset_center, runtime_center, scenario["user_id"])
        t0 = time.monotonic()
        sr = run_scenario(scenario, dispatcher, runtime_layer, audit_store, model_store)
        elapsed = (time.monotonic() - t0) * 1000

        status = "✅" if sr.total_fail == 0 else "⚠️"
        preview = ""
        if sr.turns:
            last = sr.turns[-1]
            preview = last.content_preview[:60] if last.content_preview else ""

        print(f"  [{idx+1:02d}/{len(SCENARIOS)}] {status} {sr.scenario_id} {sr.name}: "
              f"{sr.total_ok}ok/{sr.total_fail}fail, {elapsed/1000:.2f}s, last='{preview}'")
    ...
    print(f"\n{'='*70}")
    print(f"  E2E 架构测试报告 — {total_scenarios} 场景 × 20 轮 = {total_turns} 轮")
    print(f"{'='*70}")
    print(f"  总耗时:         {grand_elapsed:.1f}s")
    print(f"  场景通过:       {scenarios_all_ok}/{total_scenarios} ({scenarios_all_ok/total_scenarios*100:.0f}%)")
    print(f"  场景有失败:     {scenarios_with_fail}/{total_scenarios}")
    print(f"  总轮次:         {total_turns}")
    print(f"  成功轮次:       {total_ok} ({total_ok/total_turns*100:.1f}%)")
    print(f"  失败轮次:       {total_fail} ({total_fail/total_turns*100:.1f}%)")
    print(f"  平均轮次耗时:   {grand_elapsed/total_turns*1000:.0f}ms")

    # Architecture verification
    bindings = asset_center.list_session_bindings()
    # bindings can be AssetSessionBindingRecord objects or dicts
    def _get(b, key, default=None):
        if isinstance(b, dict):
            return b.get(key, default)
        return getattr(b, key, default)

    unique_sessions = len(set(_get(b, "upstream_session_id") for b in bindings))

    user_sessions = {}
    for b in bindings:
        uid = _get(b, "asset_id", "unknown")
        user_sessions.setdefault(uid, set()).add(_get(b, "local_session_id", ""))
    isolated = all(len(sessions) >= 1 for sessions in user_sessions.values())

    multi_hop = sum(1 for b in bindings if _get(b, "parent_session_id") is not None)

    modes = {}
    for b in bindings:
        meta = _get(b, "metadata", {}) or {}
        mode = meta.get("resolution_mode", "new")
        modes[mode] = modes.get(mode, 0) + 1

    print(f"\n  Phase P 架构验证:")
    print(f"  ├─ Audit records:         {len(audit_store.list_records())}")
    print(f"  ├─ Model records:         {len(model_store._records)}")
    print(f"  ├─ Session bindings:      {len(bindings)}")
    print(f"  ├─ Unique upstream sess:  {unique_sessions}")
    print(f"  ├─ User isolation:        {'✅' if isolated else '❌'} ({len(user_sessions)} users)")
    print(f"  ├─ Multi-hop bindings:    {multi_hop}")
    print(f"  ├─ Binding modes:         {modes}")
    print(f"  ├─ Runtime assets:        {len(runtime_center.list_all())}")
    ...
    if failed_scenarios:
        ...
        for r in failed_scenarios:
            ...
            for t in r.turns:
                if not t.ok:
                    print(f"      Turn {t.turn_index}: '{t.message[:50]}' → {t.error}")

    # Save report
    report = {
        "test_type": "architecture_level_e2e",
        ...
        "avg_turn_ms": round(grand_elapsed / total_turns * 1000, 0),
        "architecture_verification": {
            "audit_records": len(audit_store.list_records()),
            "model_records": len(model_store._records),
            "session_bindings": len(bindings),
            "unique_sessions": unique_sessions,
            "user_isolation": isolated,
            "multi_hop_bindings": multi_hop,
            "binding_modes": modes,
            "runtime_assets": len(runtime_center.list_all()),
        },
        "details": [
            {
                "id": r.scenario_id, "name": r.name, "user_id": r.user_id,
                "total_turns": r.total_turns, "ok": r.total_ok, "fail": r.total_fail,
                "ms": round(r.total_ms, 0),
            }
            for r in all_results
        ],
    }
    report_path = Path("/tmp/agentsystem_e2e_arch_report.json")
