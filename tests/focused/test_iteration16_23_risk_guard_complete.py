"""Risk guard integration verification for Iteration 16-23.

Complete coverage of:
- Rate Limiter (receive_message entry)
- Tool Loop Guard (9 handlers + runtime asset tool)
- Observability (command metrics + block events)
- Contract Linter (tool args validation)
"""
...
class TestToolLoopGuardIntegration:
    ...
    def test_all_tool_handlers_have_guard(self):
        """Verify all tool handlers have tool loop guard coverage."""
        memory = LightBrainMemory()
        interpreter = LightBrainInterpreter()
        gateway = LightBrainGateway(memory=memory, interpreter=interpreter)
        
        handlers = [
            '_handle_runtime_asset_tool',
            '_handle_package_list_installed',
            '_handle_package_show',
            '_handle_package_build',
            '_handle_package_install',
            '_handle_package_uninstall',
            '_handle_package_rollback',
            '_handle_package_search',
            '_handle_master_execute',
        ]
        ...
        for handler_name in handlers:
            ...
            assert 'check_allowed' in source and 'record_call' in source, \
                f"{handler_name} should have tool loop guard checks"


class TestContractLinterIntegration:
    """Verify contract linter is integrated into tool execution path."""

    def test_contract_linter_instance_exists(self):
        """Verify contract linter is instantiated in gateway."""
        memory = LightBrainMemory()
        interpreter = LightBrainInterpreter()
        gateway = LightBrainGateway(memory=memory, interpreter=interpreter)
    ...
    def test_contract_linter_in_runtime_asset_tool(self):
        """Verify contract linter is called in _handle_runtime_asset_tool."""
        import inspect
        ...
        handler = gateway._handle_runtime_asset_tool
        source = inspect.getsource(handler)
        
        assert 'validate_tool_args' in source, "_handle_runtime_asset_tool should call contract linter"
...
class TestRiskGuardIntegrationComplete:
    ...
    def test_all_guards_present_in_gateway(self):
        ...
        assert hasattr(gateway, '_contract_linter'), "ContractLinter should be present"
    ...
    def test_integration_summary(self):
        ...
        - ToolLoopGuard: reset_command() at command start
                        check_allowed() / record_call() in ALL tool handlers:
                          * _handle_runtime_asset_tool
                          * _handle_package_list_installed
                          * _handle_package_show
                          * _handle_package_build
                          ...
                          * _handle_package_search
                          * _handle_master_execute
        - ContractLinter: validate_tool_args() in _handle_runtime_asset_tool
        ...
        - Tool loop prevention: Per-command tool call limits enforced (9 handlers)
        - Contract validation: Tool args validated before execution
        - Observability: All commands recorded with metrics + block events
        """
