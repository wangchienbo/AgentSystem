...
def test_compare_generated_skill_reports_tag_schema_and_risk_deltas() -> None:
    create_response = client.post(
        "/skills/create",
        json={
            "skill_id": "skill.compare.delta.target",
            "name": "Compare Delta Target",
            "description": "baseline compare target",
            "adapter_kind": "callable",
            "generation_operation": "extract_text_metadata",
            "tags": ["text", "metadata"],
            "manifest_risk": {
            ...
            "smoke_test_inputs": {"text": "Compare Base 2026"}
        },
    )
    assert create_response.status_code == 200

    revise_response = client.post(
        "/skills/skill.compare.delta.target/revise",
        json={
            "version": "1.1.0",
            "description": "delta compare target revised",
            "generation_operation": "extract_text_metadata",
            "tags": ["text", "metadata", "reviewed"],
            "manifest_risk": {
                "risk_level": "R1_filesystem_write",
                "allow_shell": False,
                "allow_network": False,
            ...
            "smoke_test_inputs": {"text": "Compare Updated 2027"},
            "note": "delta-rich revision"
        },
    )
    assert revise_response.status_code == 200

    compare = client.get(
        "/skills/skill.compare.delta.target/compare",
        params={"from_version": "1.0.0", "to_version": "1.1.0"},
    )
    assert compare.status_code == 200
    payload = compare.json()
    assert payload["tags_changed"] is True
    assert payload["tags_added"] == ["reviewed"]
    assert payload["tags_removed"] == []
    assert payload["risk_level_changed"] is True
    assert payload["manifest_risk_changed"] is True
    assert payload["input_schema_changed"] is True
    assert payload["output_schema_changed"] is True
    assert payload["change_count"] >= 4
    assert payload["summary"].startswith("Changed:")
