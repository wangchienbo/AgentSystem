...
def test_api_action_success_shape_matches_chat_contract() -> None:
    user_sessions.clear()
    conversation_history.clear()
    user_sessions["session_tester"] = {
        "username": "tester",
    ...
    fake_reply = ChatMessageResponse(type="progress", content="ok", session_id="session_tester")
    ...
    assert data["actions"] == []
    assert data["related_app"] is None
