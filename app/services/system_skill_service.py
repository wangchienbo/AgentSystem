...
class SystemStateService:
    ...
    def execute(self, app_instance_id: str, request: SystemStateRequest) -> SystemStateResponse:
        ...
        if request.operation == "get":
            return SystemStateResponse(
                ...
                operation="get",
                key=request.key,
                value=deepcopy(state.get(request.key)),
    ...
    def _persist(self, app_instance_id: str) -> None:
        self._data_store.put_record(
            ...
            key="system_state",
            value={"values": self._state.get(app_instance_id, {})},
            tags=["system", "state"],
        )
        if self._store is not None:
            payload = {key: value for key, value in self._state.items()}
            self._store._write_json("system_state_runtime", payload)
...
class SystemAuditService:
    ...
    def _persist(self, app_instance_id: str) -> None:
        self._data_store.put_record(
            ...
            key="system_audit",
            value={"entries": [item.model_dump(mode="json") for item in self._records.get(app_instance_id, [])]},
            tags=["system", "audit"],
        )
        if self._store is not None:
