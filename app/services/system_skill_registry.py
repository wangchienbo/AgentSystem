from __future__ import annotations

from collections.abc import Callable

from app.models.skill_control import SkillCapabilityProfile, SkillRegistryEntry, SkillVersion
from app.models.skill_runtime import SkillExecutionRequest, SkillExecutionResult
from app.services.skill_authoring import SkillAuthoringService
from app.services.skill_control import SkillControlService
from app.services.skill_runtime import SkillRuntimeService

SkillHandler = Callable[[SkillExecutionRequest], SkillExecutionResult]


authoring = SkillAuthoringService()


SYSTEM_SKILL_SPECS: dict[str, dict] = {
    "skill.echo": {
        "name": "Demo Echo Skill",
        "immutable_interface": False,
        "version": "1.0.0",
        "content": "demo echo handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="C1_optional_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R0_safe_read",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="skill.echo",
            name="Demo Echo Skill",
            handler_entry="app.api.main:_demo_echo_skill",
            description="Simple deterministic echo skill",
            tags=["demo", "deterministic"],
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="C1_optional_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R0_safe_read",
            ),
            content="demo echo handler",
        ).manifest,
    },
    "system.app_config": {
        "name": "System App Config",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "system app config handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.app_config",
            name="System App Config",
            handler_entry="app.api.main:_system_app_config_skill",
            description="Deterministic per-app configuration access",
            input_schema_ref="schema://system.app_config/input",
            output_schema_ref="schema://system.app_config/output",
            error_schema_ref="schema://system.app_config/error",
            tags=["system", "config"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="system app config handler",
        ).manifest,
    },
    "system.state": {
        "name": "System State",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "system state handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.state",
            name="System State",
            handler_entry="app.api.main:_system_state_skill",
            description="Deterministic runtime state access",
            tags=["system", "state"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="system state handler",
        ).manifest,
    },
    "system.audit": {
        "name": "System Audit",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "system audit handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.audit",
            name="System Audit",
            handler_entry="app.api.main:_system_audit_skill",
            description="Structured audit trail recording",
            tags=["system", "audit"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="system audit handler",
        ).manifest,
    },
    "system.context": {
        "name": "System Context",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "system context handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.context",
            name="System Context",
            handler_entry="app.api.main:_system_context_skill",
            description="Deterministic shared context access",
            input_schema_ref="schema://system.context/input",
            output_schema_ref="schema://system.context/output",
            error_schema_ref="schema://system.context/error",
            tags=["system", "context"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="system context handler",
        ).manifest,
    },
    "system.meta_app": {
        "name": "System Meta App",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "system meta app bootstrap handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.meta_app",
            name="System Meta App",
            handler_entry="app.bootstrap.skills:system_meta_app_skill",
            description="Bootstrap app-scoped control structures compatible with current app generation and modification flows",
            input_schema_ref="schema://system.meta_app/input",
            output_schema_ref="schema://system.meta_app/output",
            error_schema_ref="schema://system.meta_app/error",
            tags=["system", "meta-app", "bootstrap"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="system meta app bootstrap handler",
        ).manifest,
    },
    "model.responses.probe": {
        "name": "Model Responses Probe",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "external model probe handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N2_required",
            runtime_criticality="C1_optional_runtime",
            execution_locality="remote",
            invocation_default="automatic",
            risk_level="R2_network_call",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="model.responses.probe",
            name="Model Responses Probe",
            handler_entry="app.bootstrap.skills:model_responses_probe_skill",
            description="Calls an OpenAI-compatible responses API and returns a normalized probe result",
            input_schema_ref="schema://model.responses.probe/input",
            output_schema_ref="schema://model.responses.probe/output",
            error_schema_ref="schema://model.responses.probe/error",
            tags=["model", "external", "probe"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N2_required",
                runtime_criticality="C1_optional_runtime",
                execution_locality="remote",
                invocation_default="automatic",
                risk_level="R2_network_call",
            ),
            content="external model probe handler",
        ).manifest,
    },
    "requirement.skill": {
        "name": "Requirement Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "requirement capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="requirement.skill",
            name="Requirement Capability Skill",
            handler_entry="app.bootstrap.skills:requirement_capability_skill",
            description="Requirement clarify/extract/readiness/blueprint draft surface",
            input_schema_ref="schema://requirement.skill/input",
            output_schema_ref="schema://requirement.skill/output",
            error_schema_ref="schema://requirement.skill/error",
            tags=["system", "requirement", "capability"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="requirement capability handler",
        ).manifest,
    },
    "evidence.skill": {
        "name": "Evidence Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "evidence capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R0_safe_read",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="evidence.skill",
            name="Evidence Capability Skill",
            handler_entry="app.bootstrap.skills:evidence_capability_skill",
            description="Evidence list/stats/context summary surface",
            input_schema_ref="schema://evidence.skill/input",
            output_schema_ref="schema://evidence.skill/output",
            error_schema_ref="schema://evidence.skill/error",
            tags=["system", "evidence", "retrieval"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R0_safe_read",
            ),
            content="evidence capability handler",
        ).manifest,
    },
    "context.compaction.skill": {
        "name": "Context Compaction Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "context compaction capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="context.compaction.skill",
            name="Context Compaction Capability Skill",
            handler_entry="app.bootstrap.skills:context_compaction_capability_skill",
            description="Context compact/working-set/layer surface",
            input_schema_ref="schema://context.compaction.skill/input",
            output_schema_ref="schema://context.compaction.skill/output",
            error_schema_ref="schema://context.compaction.skill/error",
            tags=["system", "context", "compaction"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="context compaction capability handler",
        ).manifest,
    },
    "workflow.insight.skill": {
        "name": "Workflow Insight Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "workflow insight capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R0_safe_read",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="workflow.insight.skill",
            name="Workflow Insight Capability Skill",
            handler_entry="app.bootstrap.skills:workflow_insight_capability_skill",
            description="Workflow overview/timeline/stats/dashboard surface",
            input_schema_ref="schema://workflow.insight.skill/input",
            output_schema_ref="schema://workflow.insight.skill/output",
            error_schema_ref="schema://workflow.insight.skill/error",
            tags=["system", "workflow", "insight"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R0_safe_read",
            ),
            content="workflow insight capability handler",
        ).manifest,
    },
    "risk.governance.skill": {
        "name": "Risk Governance Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "risk governance capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="build_and_runtime_governance",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="risk.governance.skill",
            name="Risk Governance Capability Skill",
            handler_entry="app.bootstrap.skills:risk_governance_capability_skill",
            description="Risk governance events/stats/dashboard/override surface",
            input_schema_ref="schema://risk.governance.skill/input",
            output_schema_ref="schema://risk.governance.skill/output",
            error_schema_ref="schema://risk.governance.skill/error",
            tags=["system", "risk", "governance"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="build_and_runtime_governance",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="risk governance capability handler",
        ).manifest,
    },
    "prompt.selection.skill": {
        "name": "Prompt Selection Capability Skill",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "prompt selection capability handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N0_none",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R0_safe_read",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="prompt.selection.skill",
            name="Prompt Selection Capability Skill",
            handler_entry="app.bootstrap.skills:prompt_selection_capability_skill",
            description="Prompt evidence search and prompt selection surface",
            input_schema_ref="schema://prompt.selection.skill/input",
            output_schema_ref="schema://prompt.selection.skill/output",
            error_schema_ref="schema://prompt.selection.skill/error",
            tags=["system", "prompt", "selection"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N0_none",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R0_safe_read",
            ),
            content="prompt selection capability handler",
        ).manifest,
    },
    "core.skill.control": {
        "name": "Human Skill Control Interface",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "protected control surface",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L0_deterministic",
            network_requirement="N0_none",
            runtime_criticality="build_and_runtime_governance",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R1_local_write",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="core.skill.control",
            name="Human Skill Control Interface",
            handler_entry="app.services.skill_control:SkillControlService",
            description="Protected control surface for skill lifecycle",
            tags=["system", "governance"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L0_deterministic",
                network_requirement="N0_none",
                runtime_criticality="build_and_runtime_governance",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R1_local_write",
            ),
            content="protected control surface",
        ).manifest,
    },
}


def build_registry_entry(skill_id: str) -> SkillRegistryEntry:
    spec = SYSTEM_SKILL_SPECS[skill_id]
    return SkillRegistryEntry(
        skill_id=skill_id,
        name=spec["name"],
        origin="builtin",
        immutable_interface=spec["immutable_interface"],
        active_version=spec["version"],
        versions=[SkillVersion(version=spec["version"], content=spec["content"])],
        dependencies=[],
        capability_profile=spec["capability_profile"],
        runtime_adapter=spec["manifest"].runtime_adapter,
        manifest=spec["manifest"],
    )


def register_builtin_skills(skill_control: SkillControlService) -> None:
    for skill_id in SYSTEM_SKILL_SPECS:
        skill_control.register(build_registry_entry(skill_id))


def register_builtin_handlers(skill_runtime: SkillRuntimeService, handlers: dict[str, SkillHandler], skill_control: SkillControlService) -> None:
    for skill_id, handler in handlers.items():
        entry = skill_control.get_skill(skill_id) if skill_id in SYSTEM_SKILL_SPECS else None
        skill_runtime.register_handler(skill_id, handler, entry=entry)
    "system.maoxuan": {
        "name": "Mao Zedong Thinking Framework",
        "immutable_interface": True,
        "version": "1.0.0",
        "content": "mao zedong perspective analysis handler",
        "capability_profile": SkillCapabilityProfile(
            intelligence_level="L1_assisted",
            network_requirement="N2_required",
            runtime_criticality="C2_required_runtime",
            execution_locality="local",
            invocation_default="automatic",
            risk_level="R0_safe_read",
        ),
        "manifest": authoring.build_callable_entry(
            skill_id="system.maoxuan",
            name="Mao Zedong Thinking Framework",
            handler_entry="app.bootstrap.skills:maoxuan_capability_skill",
            description="Analyze problems using Mao Zedong's thinking framework from Mao Xuan Selected Works",
            input_schema_ref="schema://system.maoxuan/input",
            output_schema_ref="schema://system.maoxuan/output",
            error_schema_ref="schema://system.maoxuan/error",
            tags=["system", "maoxuan", "analysis", "strategic-thinking"],
            immutable_interface=True,
            capability_profile=SkillCapabilityProfile(
                intelligence_level="L1_assisted",
                network_requirement="N2_required",
                runtime_criticality="C2_required_runtime",
                execution_locality="local",
                invocation_default="automatic",
                risk_level="R0_safe_read",
            ),
            content="mao zedong perspective analysis handler",
        ).manifest,
    },
