...
class AppConfigService:
    ...
    def execute(self, app_instance_id: str, request: AppConfigRequest) -> AppConfigResponse:
        ...
        if request.operation == "set":
            ...
            self._history.setdefault(app_instance_id, []).append(
                AppConfigMutation(app_instance_id=app_instance_id, action="set", key=request.key, value=deepcopy(request.value))
            )
            self._persist_snapshot(app_instance_id)
            return AppConfigResponse(
                app_instance_id=app_instance_id,
        ...
        if request.operation == "patch":
            ...
            self._history.setdefault(app_instance_id, []).append(
                AppConfigMutation(app_instance_id=app_instance_id, action="patch", key=request.key, value=deepcopy(request.value))
            )
            self._persist_snapshot(app_instance_id)
            return AppConfigResponse(
                app_instance_id=app_instance_id,
        ...
        if request.operation == "delete":
            ...
            self._history.setdefault(app_instance_id, []).append(
                AppConfigMutation(app_instance_id=app_instance_id, action="delete", key=request.key, value=deepcopy(removed))
            )
            self._persist_snapshot(app_instance_id)
            return AppConfigResponse(
                app_instance_id=app_instance_id,
    ...
    def _persist_snapshot(self, app_instance_id: str) -> None:
        ...
        self._data_store.put_record(
            ...
            key="app_config",
            value={"values": snapshot.values, "config_schema": snapshot.config_schema, "updated_at": snapshot.updated_at.isoformat()},
            tags=["system", "config"],
        )
        self._data_store.put_record(
            namespace_id=f"{app_instance_id}:system_metadata",
            key="app_config_history",
            value={"entries": [item.model_dump(mode="json") for item in self._history.get(app_instance_id, [])]},
            tags=["system", "config", "history"],
        )
