...
def _build_message_from_choice(choice: dict[str, Any]) -> dict[str, Any]:
    ...
    # DeepSeek may put reasoning_content at message or choice level
    # Check BOTH to be safe
    msg_rc = normalized.get("reasoning_content")
    if not msg_rc and isinstance(choice, dict):
        ...
        *,
        ...
        with httpx.Client(timeout=_build_timeout(effective_timeout_seconds)) as client:
            ...
            for attempt in range(max_attempts):
                try:
                    response = client.post(url, json=payload, headers=headers)
                    if _is_transient_status(response.status_code) and attempt < max_attempts - 1:
    ...
    def chat_turns(
        ...
        *,
        ...
        for turn in range(max_turns):
            ...
            # passed back on subsequent turns. The API requires ALL assistant
            # messages in reasoning mode to have the reasoning_content field,
            # even when the model's original response omitted it (empty string).
