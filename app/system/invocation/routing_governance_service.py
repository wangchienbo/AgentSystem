...
class RoutingGovernanceService:
    def __init__(
        ...
        *,
        ...
        dispatcher: InvocationDispatcher | None = None,
    ) -> None:
    ...
    def allocate_port(self, target_id: str, *, host: str = "127.0.0.1", preferred_port: int | None = None, port_range: range | None = None) -> PortAllocationRecord:
        return self._registry.allocate_port(target_id, host=host, preferred_port=preferred_port, port_range=port_range)
    ...
    def resolve_route(self, query: str) -> dict[str, object]:
        ...
        port = self._registry.get_port_allocation(target_id)
        return {
            "target_id": target_id,
            "runtime": None if runtime is None else runtime.__dict__,
            "endpoint": None if endpoint is None else endpoint.__dict__,
            "port": None if port is None else port.__dict__,
            "runtime_asset": None if runtime_asset is None else runtime_asset.model_dump(mode="json"),
        }

    def dispatch_via_route(self, query: str, *, method: str, params: dict | None = None) -> dict[str, object]:
        if self._dispatcher is None:
            raise ValueError("dispatcher is required for dispatch_via_route")
        target_id = self.resolve_target_id(query)
        route = self.resolve_route(query)
        result = self._dispatcher.dispatch(asset_id=target_id, method=method, params=params or {})
        return {
            "route": route,
            "dispatch": result,
        }
