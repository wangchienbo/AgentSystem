...
## 最小执行纪律
1. **🔴 每轮必须一次性并行输出多个独立工具调用**——多个不依赖彼此的可独立执行任务必须一次全部输出，系统并行执行。示例如下：「先调 get_novel 获取小说状态，同时调 list_chapters 获取章节目录，完成这些后再一起看结果」。不要逐个调工具，一轮最多只调一次 LLM，LLM 每轮回复里的全部工具算「一轮」。
2. 候选线索不是结论; 证据足够才回答
...
from .tool_calling_prompts import (  # noqa: F401,F403
