...
def register_expense_tracker_worker(runtime_services: dict) -> None:
    ...
    try:
        ...
        # 短名：目录 slug（LLM 调度时可能用短名做 app 参数）
        master_control.register_app_worker("expense_tracker", worker)

        logger.info("✅ expense_tracker Worker registered (keys: bp.designed.personal-finance-tracker / personal-finance-tracker / personal_finance_tracker / 个人财务记账助手 / expense_tracker)")
