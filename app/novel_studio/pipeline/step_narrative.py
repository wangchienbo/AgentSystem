...
class NarrativeModule(BaseModule):
    ...
    async def execute(self, ctx: PipelineContext) -> PipelineContext:
        ...
        system_prompt = (
            f"你是一位逻辑严谨的中国古典小说作家，正在创作小说《{novel.title}》。"
            f"请严格遵循以下要求：\n"
            f"1. 常识合理性：每个角色行为的动机和结果都必须合理。如果有人付钱买东西，那东西必须是有实际价值且能带走的实物；如果有人在服务行业讨生活，必须考虑顾客是否真的会为此付费。\n"
            f"2. 因果逻辑：角色做A事，必须有合理的原因导致B结果。不能出现无缘无故的收益或损失。\n"
            f"3. 物理合理性：角色在物理世界中的行动（如画画、搬运、建造）必须符合基本常识——画在沙子上别人带不走就不会付钱，折断树枝做笔会引来注意，等等。\n"
            f"4. 社会合理性：考虑故事时代背景下的社会规则——谁有话语权、谁付钱、谁有动机。\n"
            f"5. 动机关联：每个情节转折必须由角色动机驱动，而不是为了推动情节而凭空出现。\n"
            f"在动笔写每一段前，先在心里做一次常识校验：\"这个桥段在现实中真的会有人这样做吗？\" 如果答案是否定的，重新构思。\n"
            f"使用中文写作。"
        )
