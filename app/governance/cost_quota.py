...
class CostQuotaManager:
    ...
    def __init__(self, config: QuotaConfig | None = None) -> None:
        ...
        self.resource_budget_manager = None
        self._initialize_buckets()

    def _initialize_buckets(self) -> None:
        self._set_bucket("llm_call_hourly", QuotaBucket("llm_call", self._config.llm_call_hourly, "hourly"))
        self._set_bucket("tool_call_hourly", QuotaBucket("tool_call", self._config.tool_call_hourly, "hourly"))
        self._set_bucket("llm_call_daily", QuotaBucket("llm_call", self._config.llm_call_daily, "daily"))
        self._set_bucket("tool_call_daily", QuotaBucket("tool_call", self._config.tool_call_daily, "daily"))
        self._set_bucket("app_create_daily", QuotaBucket("app_create", self._config.app_create_daily, "daily"))
        self._set_bucket("app_modify_daily", QuotaBucket("app_modify", self._config.app_modify_daily, "daily"))
        self._set_bucket("app_delete_daily", QuotaBucket("app_delete", self._config.app_delete_daily, "daily"))
    ...
    def consume(self, quota_type: QuotaType, amount: int = 1, period: QuotaPeriod = "hourly") -> tuple[bool, QuotaBucket]:
        ...
        if bucket.current + amount > bucket.limit:
            raise QuotaExceededError(quota_type=quota_type, current=bucket.current, limit=bucket.limit, period=period)
        bucket.current += amount
        return True, bucket

    def get_usage(self, quota_type: QuotaType | None = None) -> dict[str, QuotaBucket]:
        if quota_type:
            return {k: v for k, v in self._buckets.items() if v.quota_type == quota_type}
    ...
    def _get_resource_consumption(self) -> dict:
        manager = getattr(self, "resource_budget_manager", None)
        if manager is None:
            return {}
        return getattr(manager, "get_usage_summary", lambda: {})()
