...
- [x] 已完成第三十七个最小改造点：active skill 中 lifecycle/query/list 等多轮恢复已改为经 `AppCommandRecoveryService.recover_command(..., force_confirmed=False)` 统一重建 command，再委托 `AppApplicationService.handle(...)`；gateway 中 app-domain 多轮恢复细节进一步从手工拼装转向 recovery service。
- [x] 已完成第三十八个最小改造点：`AppCommandRecoveryService` 新增 `recover_from_source(...)` 与内部 `_recover_from_normalized(...)`，开始显式区分 `confirm` / `active_skill` 等恢复来源；gateway 对恢复来源差异的感知继续下沉到 recovery service。
...
- [ ] 下一步：继续把 session replay / action replay source contract 接入 `recover_from_source(...)`，完成恢复入口协议化
