...
- executable skill adapter 应验证 JSON stdin/stdout 协议、entrypoint 缺失、超时、stderr 诊断、stdout 非 JSON 等失败映射行为
- script skill generator 应验证生成目录包含 manifest/schema/entrypoint/smoke-test，并能被 registry/runtime/app installer 接入后通过 workflow skill step 执行
