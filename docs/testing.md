...
- interpreter/gateway draft-continuation coverage should validate not only staged `继续` advancement and lifecycle handoff payloads, but also executable `apply_draft_app` action routing into the application layer, runtime install/start convergence, and assistant-reply persistence for the final activation handoff
- interpreter/gateway hot-tool coverage should explicitly validate route-local prompt-tool and exec-tool alignment, especially for script-first fallback and self-iteration asset-first branches, so hot-tool sessions cannot silently drift between what the model sees and what the executor can actually call
...
- executable workflow-action coverage for solution review, task-list, repo-context, implementation, and acceptance action slices, including bounded live `/api/action` chain checks for richer repo-truth, implementation validation-map, changed-file intent source hints, distinct multi-command work-item binding, compact change-execution summary surfacing, normalized acceptance-evidence payloads, and persisted `evidence_summary` behavior
...
- trigger-side live observation consumption coverage, including merged observation digests and service-session propagation into nightly/manual governance execution
...
- authoritative app-config source-path coverage via focused config and uninstall-cleanup validation, asserting tracked `AppConfigService` implementation continues to support init/read/delete behavior without relying on ignored `app/skills/` implementation files
