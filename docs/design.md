...
- the next-phase integration bridge for app generation must register auto-materialized blueprints into `AppRegistryService` before handing them to the real installer; otherwise confirm-step install continuation is only valid with mocks and breaks against the actual registry-backed install path
