...
- app-generation hardening for the current phase is considered closed once the system can generate design output, continue through confirm -> blueprint -> install handoff, expose structured success/partial-failure state, and prove that path with both focused success and failure validation slices; deeper registry/runtime integration and caller-aware install identity are next-phase concerns rather than blockers for this slice
