...
- the next adjacent presentation domain after runtime assets is package/app management; start there with shared list-oriented presenters only, because installed-package lists, package-search results, and app lists share stable row-based rendering while package detail/build/install flows are still more heterogeneous
