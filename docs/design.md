...
- app-generation closure must not silently swallow blueprint/install failures after the confirm step; partial success is acceptable, but it has to be surfaced as explicit structured state so operators and upper layers can tell the difference between "not attempted", "succeeded", and "attempted but failed"
