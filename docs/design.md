...
- app generation is expected to behave as a staged closure rather than a pure design mock: intent analysis and architecture design produce a confirmable plan, and the confirm step should be able to continue into blueprint materialization and install hooks when those collaborators are available
