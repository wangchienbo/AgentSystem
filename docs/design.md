...
- when app install depends on skills that exist in `SkillControlService` but have not yet been materialized into AssetCenter `source/`, the installer now performs a minimal on-demand bridge: it emits discoverable skill asset source manifests from registry metadata before AssetCenter build/install resolution runs
- this keeps the identifier-space split explicit: blueprint/runtime continue using skill ids, while AssetCenter dependency resolution uses asset ids backed by discoverable source assets
