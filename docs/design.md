...
- lifecycle deletion is now a first-class `AppLifecycleService.delete_app()` capability rather than an installer-side mutation of private lifecycle storage
- installer uninstall flow now depends on the formal lifecycle API, which also matches existing worker-side expectations that `delete_app()` exists
