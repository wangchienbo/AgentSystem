...
- when app install needs skill asset sources, the bridge now prefers promoted `skill_assets/core` artifacts as the upstream source of truth and adapts them into AssetCenter-compatible `source/skill.*` manifests
- only when no core skill asset exists does the installer fall back to a minimal registry-derived materialization from `SkillControlService`
- this establishes an explicit precedence order across the two asset lifecycles and prevents AssetCenter source generation from drifting ahead of already-promoted core skill assets
