...
- uninstall now performs adjacent state cleanup, not just asset/lifecycle cleanup: it removes the app's system catalog entry, shared context, config snapshot/history, and app namespaces so delete semantics do not leave durable per-app state behind
