...
- before over-generalizing strategy presentation, extract only the low-risk runtime-asset formatting primitives that are clearly repeated today: summary-list assembly, detail-header assembly, and key-value fallback rendering; keep domain-specific phrasing and priority policy in the domain formatter until a second concrete consumer appears
