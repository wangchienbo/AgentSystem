...
- once package list and package detail rendering are shared, the next stable reuse cluster is package-operation success copy (`build` / `install` / `rollback`), which can be normalized without prematurely folding in shorter uninstall or error paths
