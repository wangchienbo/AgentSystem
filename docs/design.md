...
- generic runtime-asset detail-document presentation should reuse shared interface-detail rendering helpers for the repeated structure of method name, description, input schema, and output schema, while richer semantic annotations such as risk or runtime notes can remain domain/tool specific until multiple consumers need the same view
