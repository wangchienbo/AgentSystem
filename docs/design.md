...
- a second concrete consumer for the shared runtime-asset formatter layer is the asset visibility/prompt catalog path, where asset ids and callable methods are summarized for LLM discovery; this proves the shared module should cover both operator-facing replies and prompt-facing runtime-asset catalogs
