...
- app lifecycle operations now use the same blueprint-derived app asset identity across install, upgrade, catalog registration, and AssetCenter uninstall; `app_instance_id` remains the runtime identity, not the installable asset identity
- upgrade now returns both old/new asset ids and removes the previously installed app asset when the blueprint identity changes
