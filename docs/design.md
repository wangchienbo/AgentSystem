...
- package/app management modularization should continue by separating row-based list presenters from detail-style presenters; `package_show` is the first clean detail boundary because it renders stable package metadata plus build history without yet forcing build/install/rollback success-copy unification
