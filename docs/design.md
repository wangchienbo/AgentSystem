...
- app generation now needs a dedicated deterministic bridge from design output to installable blueprint form; requirement-derived blueprint builders are not the right contract because app-designer output already contains control-skill, subordinate-skill, decomposition, and governance structure that should map directly into an AppBlueprint
