...
- app-generation validation should include a focused acceptance slice that runs confirmation, real design-to-blueprint materialization, install handoff, and structured closure assertions together, so the chain is proven as a staged whole rather than only via isolated mocks
